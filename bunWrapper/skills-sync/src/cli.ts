...
const syncCommand: Subcommand = {
  name: 'sync',
  synopsis: 'skills-sync sync',
  blurb: 'WRITER. Regenerate the vendored tree. Setup and manual use only.',
  usage: () => {
...
It must NEVER run from a hook (D1) and refuses when it detects one. Hooks run
'skills-sync check --tier pre-commit', which is read-only.

Options:
  --help, -h      Print this message.`);
  },
  run: args => {
    rejectUnknownOptions('sync', args, ['--help', '-h']);
    if (args.includes('--help') || args.includes('-h')) {
      syncCommand.usage();
      return EXIT_OK;
    }
